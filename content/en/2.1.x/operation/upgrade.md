...
1. Manually update **ONLY** the CRD (see [upgrade operator](helm/usage/#upgrade-operator)).
...
1. Update the Core WAAP Operator by upgrading the helm chart (ensure the CoreWaapService CustomResourceDefinition was updated, see [upgrade operator](helm/usage/#upgrade-operator)).
...
- The predefined header allow classes have changed. See
  [Header filtering](../configuration/header-filtering.md) for the full, current contents of each class.
...
- **Header filtering**<br/>
  ...
  see [Header filtering](../configuration/header-filtering.md).
