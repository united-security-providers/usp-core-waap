...
The Core WAAP components log to the standard out in JSON format and can be used by customers log stack for further processing and visualization. Metrics are exposed in Prometheus format via a dedicated Kubernetes service which is named after Core WAAP CRD with `metrics-` prefix. Service's port can be configured in `spec.operation.metrics` section of the CRD (see [API Description](../configuration/crd-doc.md) for it). The URL is `/metrics`.
