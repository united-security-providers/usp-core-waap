...
_**NOTE**: This section here is only provided for use cases where legacy `spec.crs` settings are still used instead of the current `spec.coraza` settings.
For the same section with current `spec.coraza` settings see [Large Payloads](large-payloads.md) instead.
It is generally recommended to switch to the new `spec.coraza` settings as soon as possible._
...
This section details the specific Core WAAP settings that impact the handling of large payloads. For a comprehensive description of all available settings, please refer to the [Core WAAP API Reference](../configuration/crd-doc.md).
