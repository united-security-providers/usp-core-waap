...
1. Manually update **ONLY** the CRD (see [upgrade operator](helm/#upgrade-operator)).
...
1. Update the Core WAAP Operator by upgrading the helm chart (ensure the CoreWaapService CustomResourceDefinition was updated, see [upgrade operator](helm/#upgrade-operator)).
